        ...
        Dream.findOne(query)
            ...
            if (dream) {
                if (dream.likes.find(
                    ...
                    res.status(400)
                        .json({like: 'You have already liked this dream'})
                    return 
                }
...
router.get('/:likeId', (req, res) => {
    Like.findOne({_id: req.params.likeId})
        ...
        .catch(() => res.status(404)
            .json({ nolikesfound: 'No like found with the specified id'}))
});
...
router.get('/dream/:dreamId', (req, res) => {
    Like.find({dreamId: req.params.dreamId})
        ...
        .catch(() => res.status(404)
            .json({ nolikesfound: 'This dream has no likes'}))
});
...
router.get('/user/:userId', (req, res) => {
    Like.find({userId: req.params.userId})
        ...
        .then(likes => res.json(likes))
        .catch(() => res.status(404)
            .json({ nolikesfound: 'This user has no likes'}))
});
        ...
        Like.findOne({_id: req.params.likeId})
            .then(like => {
                if (like.userId != req.user.id) {
                    res.status(400)
                        .json({ userauth: 'You can only edit your own likes'})
                } else {
                    Like.deleteOne({_id: req.params.likeId}, (err) => {
                        if (err) {
                            res.status(404)
                                .json({like: 'This like cannot be removed'})
                        }
                    })
                    .then(() => {
                        ...
                        Dream.findOneAndUpdate(
                            ...
                            (err, dream) => {
                                if (err) {
                                    res.status(404)
                                        .json({like: 'Cannot edit this like'})
                                } else {
                                    res.json ({
